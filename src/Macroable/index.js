...
class Macroable {
  ...
  static macro (name, callback) {
    if (typeof (callback) !== 'function') {
      throw new Error(`${this.name}.macro expects callback to be a function or an asyncFunction`)
    }
  ...
  static getter (name, callback, singleton = false) {
    if (typeof (callback) !== 'function') {
      throw new Error(`${this.name}.getter expects callback to be a function or an asyncFunction`)
    }
   ...
   */
  static hydrate () {
    this._macros.forEach((macro, key) => Reflect.deleteProperty(this.prototype, key))
    this._getters.forEach((macro, key) => Reflect.deleteProperty(this.prototype, key))
    this._macros = {}
    this._getters = {}
  }
}
